...
function TaxesSection({ propertyId }) {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const rows = await getPropertyTaxes(propertyId)
      setTaxes(rows)
      // Fetch attached bills per record so we can link them on each card.
      const entries = await Promise.all(
        rows.map(async t => {
          try { return [t.id, await getTaxDocuments(t.id)] } catch { return [t.id, []] }
        })
      )
      setDocsByTax(Object.fromEntries(entries))
    } catch {}
    setLoading(false)
  }, [propertyId])
  ...
  async function handleMarkPaid(id, paid) {
    setBusyPaid(id)
    try { await markTaxPaid(id, paid); await load() }
    catch (err) { alert('Error: ' + err.message) }
    finally { setBusyPaid(null) }
  }
        ...
        taxes.length === 0
          ? (
            <div className="py-6 text-center border border-dashed border-slate-200 rounded-xl">
              ...
              <p className="text-sm text-slate-400">No tax records on file.</p>
              <p className="text-xs text-slate-400 mt-1">Upload a bill or add manually using the buttons above.</p>
            </div>
          )
          : (
            <div className="space-y-4">
              {taxes.map(t => {
                const isPaid = !!t.paid_date
                const docs   = docsByTax[t.id] || []
                ...
                return (
                  <div
                    key={t.id}
                    className={`p-4 rounded-xl border ${isPaid ? 'border-slate-200 bg-white' : 'border-amber-200 bg-amber-50'}`}
                  >
                    {/* Card header: paid badge + mark-as-paid + edit/delete */}
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <div className="flex items-start gap-3">
                        {isPaid ? (
                          <div className="flex flex-col items-center">
                            <span className="px-2.5 py-1 rounded-lg bg-green-100 border border-green-300 text-green-700 text-xs font-black tracking-widest uppercase">PAID</span>
                            <span className="text-[10px] text-green-600 mt-0.5">{fmtDate(t.paid_date)}</span>
                          </div>
                        ) : (
                          <span className="px-2.5 py-1 rounded-lg bg-amber-100 border border-amber-300 text-amber-700 text-xs font-black tracking-widest uppercase">UNPAID</span>
                        )}
                        <div>
                          <p className="text-sm font-semibold text-slate-900">{t.tax_year ? `${t.tax_year} Property Taxes` : 'Property Taxes'}</p>
                          {t.taxing_authority && <p className="text-xs text-slate-500 mt-0.5">{t.taxing_authority}</p>}
                        </div>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        {isPaid ? (
                          <button
                            onClick={() => handleMarkPaid(t.id, false)}
                            disabled={busyPaid === t.id}
                            className="px-2.5 py-1 rounded-lg text-xs font-medium bg-slate-100 text-slate-500 border border-slate-200 hover:bg-slate-200 transition-colors disabled:opacity-60"
                          >
                            {busyPaid === t.id ? '…' : 'Undo'}
                          </button>
                        ) : (
                          <button
                            onClick={() => handleMarkPaid(t.id, true)}
                            disabled={busyPaid === t.id}
                            className="px-2.5 py-1 rounded-lg text-xs font-semibold bg-green-600 text-white hover:bg-green-700 transition-colors disabled:opacity-60"
                          >
                            {busyPaid === t.id ? 'Saving…' : 'Mark as Paid'}
                          </button>
                        )}
                        <button onClick={() => openEdit(t)} className="p-1.5 rounded hover:bg-white/80 text-slate-400 hover:text-slate-600 transition-colors">
                          <Pencil className="w-3.5 h-3.5" />
                        </button>
                        <button onClick={() => handleDelete(t.id)} className="p-1.5 rounded hover:bg-red-100 text-slate-400 hover:text-red-500 transition-colors">
                          <Trash2 className="w-3.5 h-3.5" />
                        </button>
                      </div>
                    </div>

                    {/* Two-column sectioned detail layout */}
                    <div className="grid grid-cols-2 gap-x-6">
                      <div>
                        <SectionHead label="BILL INFO" />
                        <Row label="Tax Year"         value={t.tax_year} />
                        <Row label="Taxing Authority" value={t.taxing_authority} />
                        <Row label="Parcel Number"    value={t.parcel_number} />
                      </div>
                      <div>
                        <SectionHead label="AMOUNTS & DATES" />
                        <Row label="Amount"      value={t.amount != null && t.amount !== '' ? fmt(t.amount) : ''} />
                        <Row label="Due Date"    value={fmtDate(t.due_date)} />
                        <Row label="Paid Amount" value={t.paid_amount != null && t.paid_amount !== '' ? fmt(t.paid_amount) : ''} />
                        <Row label="Paid Date"   value={fmtDate(t.paid_date)} className={isPaid ? 'text-green-700' : ''} />
                      </div>
                    </div>
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {tab === 'cam'         && <CamSection         propertyId={propertyId} />}
