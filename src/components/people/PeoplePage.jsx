...
import { Users, UserPlus, MoreHorizontal, Pencil, Trash2, Loader2, AlertCircle, ChevronLeft, ChevronRight } from 'lucide-react'
...
export default function PeoplePage() {
  ...
  const load = useCallback(async (s, role, dnc, pg) => {
    setFetching(true)
    try {
  ...
  useEffect(() => {
    load(search, roleFilter, dncFilter, page)
  }, [page, roleFilter, dncFilter]) // eslint-disable-line
  ...
  const handleSearch = (val) => {
    ...
    searchTimer.current = setTimeout(() => {
      setPage(0)
      load(val, roleFilter, dncFilter, 0)
    }, 300)
  }

  const handleRoleFilter = (val) => { setRole(val); setPage(0); load(search, val, dncFilter, 0) }
  const handleDncFilter  = (val) => { setDnc(val);  setPage(0); load(search, roleFilter, val, 0) }
      ...
      <TopBar
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            <select
              value={dncFilter}
              onChange={e => handleDncFilter(e.target.value)}
              className="text-sm border border-slate-200 rounded-lg px-3 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wide">City / State</th>
                    <th className="w-12" />
                  </tr>
                </thead>
                <tbody>
                  {rows.map((p, i) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-slate-600 max-w-[160px] truncate">{p.company_name || <span className="text-slate-300">—</span>}</td>
                      ...
                      <td className="px-4 py-3 text-slate-600">{[p.city, p.state].filter(Boolean).join(', ') || <span className="text-slate-300">—</span>}</td>
